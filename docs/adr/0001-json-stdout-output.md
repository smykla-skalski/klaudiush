...
2. Using `systemMessage` alone (without `permissionDecisionReason`) means the agent receives only a generic "Hook denied this tool" message and never sees the actual validation error. ([Issue #12446](https://github.com/anthropics/claude-code/issues/12446))

The original trigger for this ADR was Claude Code, but the repo now uses the same structured-JSON approach as the provider-neutral baseline. Provider-specific adapters map the normalized result into each hook provider's output contract.
