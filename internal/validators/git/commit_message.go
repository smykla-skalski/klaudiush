...
// containsClaudeAIAttribution checks for AI attribution patterns while allowing CLAUDE.md references
func (v *CommitValidator) containsClaudeAIAttribution(message string) bool {
	...
	// If "claude" appears without being part of "CLAUDE.md", block it
	// This allows: "add CLAUDE.md", "update CLAUDE.md file", etc.
	// But blocks: "claude helped", "from claude", etc.
	if strings.Contains(lower, "claude") {
		// Check if it's referring to CLAUDE.md file (case-insensitive check)
		isFileReference := strings.Contains(lower, "claude.md") ||
			strings.Contains(message, "CLAUDE.md") ||
			strings.Contains(message, "CLAUDE")

		if isFileReference {
			// Allow it - it's referring to the file
			return false
		}
		// Block it - it's not a file reference
		return true
	}
