...
See github.com/smykla-skalski/klaudiush/pull/123`
		...
		Context("when message contains AI attribution", func() {
			...
			It("should allow 'copilot' in technical context", func() {
				ctx := &hook.Context{
					EventType: hook.EventTypePreToolUse,
					ToolName:  hook.ToolTypeBash,
					ToolInput: hook.ToolInput{
						Command: `git commit -sS -a -m "feat(integration): add copilot integration"`,
					},
				}
			...
			It("should allow 'codex' in technical context", func() {
				ctx := &hook.Context{
					EventType: hook.EventTypePreToolUse,
					ToolName:  hook.ToolTypeBash,
					ToolInput: hook.ToolInput{
						Command: `git commit -sS -a -m "feat(integration): add codex integration"`,
					},
				}
			...
			It("should allow AI assistant links in technical context", func() {
				ctx := &hook.Context{
					EventType: hook.EventTypePreToolUse,
					ToolName:  hook.ToolTypeBash,
					ToolInput: hook.ToolInput{
						Command: `git commit -sS -a -m "$(cat <<'EOF'
docs(ai): document assistant setup

See [GitHub Copilot](https://github.com/features/copilot).
See [Codex](https://openai.com/codex).
EOF
)"`,
					},
				}
...
EOF
			...
			It("should fail with 'GitHub Copilot' markdown link", func() {
				ctx := &hook.Context{
					EventType: hook.EventTypePreToolUse,
					ToolName:  hook.ToolTypeBash,
					ToolInput: hook.ToolInput{
...
EOF
			...
			It("should fail with 'Codex' markdown link", func() {
				ctx := &hook.Context{
					EventType: hook.EventTypePreToolUse,
					ToolName:  hook.ToolTypeBash,
					ToolInput: hook.ToolInput{
...
EOF
			...
			It("should fail with Co-Authored-By: Copilot", func() {
				ctx := &hook.Context{
					EventType: hook.EventTypePreToolUse,
					ToolName:  hook.ToolTypeBash,
					ToolInput: hook.ToolInput{
...
EOF
			...
			It("should fail with Co-Authored-By: Codex", func() {
				ctx := &hook.Context{
					EventType: hook.EventTypePreToolUse,
					ToolName:  hook.ToolTypeBash,
					ToolInput: hook.ToolInput{
...
Signed-off-by: Test User <test@klaudiu.sh>`
	...
	Describe("Inline -F file writes", func() {
		// At PreToolUse the message file written earlier in the same command
		// chain does not exist on disk yet, so the validator must read the
		// content the parser captured rather than the (missing) file.
		It("validates a heredoc-written message file (reproduces the gap)", func() {
			cmd := "cat > tmp/commit-msg-1.txt <<'EOF'\n" +
				"this is not conventional\n" +
				"EOF\n" +
				"git add -N -- foo.txt\n" +
				"git commit -sS -F tmp/commit-msg-1.txt -- foo.txt"
		...
		It("passes a valid heredoc-written message file", func() {
			cmd := "cat > tmp/msg.txt <<'EOF'\n" +
				"perf(policy-canvas): selective reroute engine\n" +
				"EOF\n" +
		...
		It("catches AI attribution in a heredoc-written message file", func() {
			cmd := "cat > tmp/msg.txt <<'EOF'\n" +
		...
		It("catches Copilot attribution in a heredoc-written message file", func() {
			cmd := "cat > tmp/msg.txt <<'EOF'\n" +
				"feat(api): add endpoint\n\n" +
				"Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>\n" +
				"EOF\n" +
		...
		It("catches Codex attribution in a heredoc-written message file", func() {
			cmd := "cat > tmp/msg.txt <<'EOF'\n" +
				"feat(api): add endpoint\n\n" +
